...
function render(){
  ...
  if(tab==='now'){
    ...
    body.innerHTML=`<div class="mt-section-heading"><div><div class="mt-kicker">📚 ЛЕТОПИСЬ</div><h3>Сохранённые сцены</h3></div><span class="mt-counter">${batches.length}</span></div><div class="mt-info">Каждая страница привязана к исходным сообщениям. Если исходник изменится или будет свайпнут, зависимая память перестанет считаться актуальной.</div><br>${batches.slice().reverse().map((b,n)=>`<article class="mt-card mt-history-card"><div class="mt-history-number">${String(batches.length-n).padStart(2,'0')}</div><div class="mt-kicker">#${Math.min(...b.sources.map(s=>map.get(s.id).index))} — #${Math.max(...b.sources.map(s=>map.get(s.id).index))}</div><h3>${escape(b.date||'Сцена без даты')}</h3><p>${escape(b.summary)}</p><button data-batch="${attr(b.id)}">📜 Открыть исходный текст</button></article>`).join('')||'<div class="mt-empty mt-diary-empty"><span>📚</span>Летопись пока пуста.</div>'}<button data-action="unhide">👁 Показать последний скрытый диапазон</button>`;
  }else if(tab==='context'){
...
Memory Thread beta.3 · Story Journal. Основная ролевая модель получает только выбранную часть памяти в пределах заданного бюджета, а полный архив продолжает храниться отдельно.`);}
