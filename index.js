...
import {analyzeBroom,applySafeBroom} from './compaction.js';
...
function mergeRecord(id,preselected=[]){
...
function help(){modal('❔ Как работает Нить памяти',`<div class="mt-help-simple">
<h3>1. Ты играешь как обычно</h3><p>Memory Thread ничего не отправляет архивариусу сам. Напоминание после N сообщений — только подсказка. Ты сама решаешь, когда нажать «Запомнить новое» или выбрать диапазон.</p>
<h3>2. Сохранение сцены</h3><p>Только в этот момент выбранные сообщения уходят модели для памяти. Она возвращает краткую сцену, общий сюжет и отдельные воспоминания. После успешного ответа они сохраняются в текущем чате.</p>
<h3>3. Что получает ролевая модель</h3><p>Не весь архив. Перед каждым обычным ответом Memory Thread локально собирает маленький «рюкзак» до установленного бюджета: последнюю сохранённую сцену, общий сюжет, закреплённые/вызванные записи и несколько подходящих воспоминаний.</p>
<h3>4. Память</h3><p>«Новая запись» — твоя ручная заметка. Она защищена от перезаписи архивариусом, но не обязана попадать в каждый ответ: закрепи её, сделай важной или вызови вручную, если хочешь гарантировать ближайшее напоминание.</p>
<h3>5. Собрать по теме</h3><p>Ищет только в уже сохранённой памяти и не тратит API. Кнопка «Вспомнить найденное» пытается положить найденные записи в следующий контекст один раз.</p>
<h3>6. Веник 🧹</h3><p>Проверяет память локально и ничего не отправляет в API. Он может безопасно убрать только точные технические дубли и старые пустые отметки. Похожие по смыслу записи он не удаляет сам: показывает их тебе и предлагает открыть обычное «Объединить», чтобы решение осталось за тобой.</p>
<h3>7. Хроника</h3><p>Главы — это удобные папки из уже сохранённых сцен. Они собираются локально, не вызывают модель и сами по себе не увеличивают контекст.</p>
<h3>8. Файлы</h3><p><b>JSON</b> — настоящая резервная копия, которую можно потом импортировать обратно. <b>HTML</b> — красивая копия дневника только для чтения в браузере, хранения или отправки подруге; обратно в Memory Thread её импортировать не нужно.</p>
<h3>9. Скрытие сообщений</h3><p>Сначала сохрани сцену, потом можешь скрыть старые сообщения в SillyTavern. Сохранённая память останется. Удалять или сильно редактировать исходные сообщения нежелательно: записи привязаны к их источникам.</p>
</div>`,()=>dialog.close(),'Понятно');}

function broomWizard(){
  requireIdle();const report=analyzeBroom(mem(),chat());
  const pairs=report.similarPairs;
  const pairHtml=pairs.length?`<div class="mt-broom-pairs">${pairs.map((p,i)=>`<label class="mt-merge-option"><input type="radio" name="pair" value="${attr(p.a.id)}|${attr(p.b.id)}"><span><strong>${escape(p.a.title)}</strong> ↔ <strong>${escape(p.b.title)}</strong><small>${escape(CATEGORIES[p.a.category])} · похожесть ${p.score}%</small></span></label>`).join('')}</div>`:'<div class="mt-info">Похожих активных записей не найдено.</div>';
  modal('🧹 Веник памяти',`<div class="mt-card mt-broom-card"><div class="mt-kicker">ЛОКАЛЬНАЯ УБОРКА · БЕЗ API</div><h3>Что нашёл веник</h3><p>Активных воспоминаний: <strong>${report.activeCount}</strong> · всего сохранённых версий: <strong>${report.totalVersions}</strong></p><div class="mt-stats"><span>🧽 точных дублей: ${report.exactDuplicateIds.length}</span><span>🧷 пустых отметок: ${report.staleDismissed.length}</span><span>🔗 похожих пар: ${pairs.length}</span></div></div><div class="mt-info">Безопасная уборка удаляет только полностью одинаковые технические копии и неиспользуемые отметки. Уникальные факты, старые версии для отката, сцены и главы она не трогает.</div><br>${report.exactDuplicateIds.length?check('exact','🧽 Убрать точные дубли',true):''}${report.staleDismissed.length?check('stale','🧷 Убрать пустые отметки отключения',true):''}<details ${pairs.length?'':'hidden'}><summary>🔗 Похожие воспоминания — проверить вручную</summary><p class="mt-sub">Выбери максимум одну пару. После уборки откроется обычное окно «Объединить», где ты ещё раз всё увидишь и сама подтвердить слияние.</p>${pairHtml}</details>`,async f=>{
    const pair=String(f.get('pair')||'');const removeExact=f.has('exact'),removeStale=f.has('stale');
    if(removeExact||removeStale)await mutate(m=>applySafeBroom(m,report,{removeExact,removeStaleDismissed:removeStale}));
    if(pair){const [a,b]=pair.split('|');setTimeout(()=>mergeRecord(a,[b]),0);}
    else notify('🧹 Уборка завершена',`Удалено точных дублей: ${removeExact?report.exactDuplicateIds.length:0}. Память и сцены не пересказывались.`,()=>openPanel('memories'));
  },'🧹 Навести порядок');
}
...
function topicWizard(){
  ...
  modal('🧭 Собрать память по теме',`${input('topic','Тема или имя','','text','required maxlength="160" placeholder="Арман, отношения Арман–Ханаби, магия…"')}<div class="mt-info">Поиск идёт только по уже сохранённой памяти и ничего не отправляет в API.</div>`,async f=>{
    ...
    dialog.innerHTML=`<form id="mt-topic-result"><h3>🧭 ${escape(q)}</h3>${matches.length?`<div class="mt-topic-results">${matches.map(r=>`<article class="mt-topic-item"><strong>${escape(r.title)}</strong><div class="mt-sub">${escape(CATEGORIES[r.category])}</div><p>${escape(r.text)}</p></article>`).join('')}</div><div class="mt-info">Найдено ${matches.length}. «Вспомнить найденное» добавит подходящие записи только к ближайшему следующему ответу модели. После ответа это ручное напоминание снимается. Общий лимит памяти всё равно соблюдается.</div>`:'<div class="mt-empty"><span>🔎</span>В сохранённой памяти ничего похожего не найдено.</div>'}<div class="mt-row mt-actions">${matches.length?'<button type="button" class="mt-primary" id="mt-topic-recall">🧠 Вспомнить найденное</button>':''}<button type="button" id="mt-topic-close">Закрыть</button></div></form>`;
