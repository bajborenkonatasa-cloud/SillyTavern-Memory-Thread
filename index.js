...
2. По умолчанию оставь «Текущее API Таверны»: расширение автоматически использует выбранные сейчас в SillyTavern провайдер и модель — URL, ключ и название модели вручную не нужны. Отдельное OpenAI-совместимое API настраивается только если ты специально хочешь другую модель для памяти; оно требует CORS, а ключ хранится только в сессии браузера.
